...
func VerifyAppName(appName string) bool {
	dokkuRoot := GetEnv("DOKKU_ROOT")
	appRoot := strings.Join([]string{dokkuRoot, appName}, "/")
	_, err := os.Stat(appRoot)
	if os.IsNotExist(err) {
		LogFail(fmt.Sprintf("App %s does not exist", appName))
	}
	r, _ := regexp.Compile("^[a-z].*")
	if !r.MatchString(appName) {
		LogFail("App name must begin with lowercase alphanumeric character")
	}
	return true
}

// GetEnv returns env variable or fails if it's not set
...
func GetDeployingAppImageName(args ...string) (imageName string) {
	...
	if appName == "" {
		LogFail("(GetDeployingAppImageName) APP must not be null")
	}
